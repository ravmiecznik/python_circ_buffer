...
def test_get_from_empty_buffer(cbuffer):
    ...
    assert cbuffer.get() is None
    assert cbuffer.gets() is None
...
def test_flush(cbuffer_size10):
    ...
    assert cbuffer_size10.get() is None
    assert cbuffer_size10.gets() is None
...
def test_cbuffer_get_as_list_method(cbuffer):
    ...
    assert cbuffer.gets_list() is None
...
def test_cbuffer_get_as_list_get_more_than_available(cbuffer):
    test_string = "test string"
    cbuffer.puts(test_string)
